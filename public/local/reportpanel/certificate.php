...
/**
 * Certificate reports — issued counts (full mode) or own certificates (self mode).
 *
...
use local_reportpanel\helper\certreport;
...
$PAGE->set_url(new moodle_url('/local/reportpanel/certificate.php'));
$PAGE->set_pagelayout('standard');
$PAGE->set_title(get_string('cert_title', 'local_reportpanel'));
$PAGE->set_heading(get_string('cert_title', 'local_reportpanel'));

$backurl = (new moodle_url('/local/reportpanel/index.php'))->out(false);
$full = access::is_full_mode();
$installed = certreport::available();
...
if ($full) {
    ...
    if ($courseid) {
        ...
        if ($course) {
            $rows = certreport::course_cert_stats($courseid);
        }
    }

    $templatecontext = [
        'backurl' => $backurl,
        'intro' => get_string('cert_intro_full', 'local_reportpanel'),
        'actionurl' => (new moodle_url('/local/reportpanel/certificate.php'))->out(false),
        'installed' => $installed,
...
");
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->render_from_template('local_reportpanel/cert_full', $templatecontext);
    echo $OUTPUT->footer();
} else {
    $rows = certreport::own_certificates((int)$USER->id);
    $templatecontext = [
        'backurl' => $backurl,
        'intro' => get_string('cert_intro_self', 'local_reportpanel'),
        'installed' => $installed,
        'rows' => $rows,
        'hasrows' => !empty($rows),
    ];
    echo $OUTPUT->header();
    echo $OUTPUT->render_from_template('local_reportpanel/cert_self', $templatecontext);
    echo $OUTPUT->footer();
}
