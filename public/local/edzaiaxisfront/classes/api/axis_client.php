...
class axis_client_exception extends \moodle_exception
{
    ...
    /** Raw underlying detail (network/API error text), without the friendly prefix. */
    public string $detail;
    public function __construct(string $message, int $http_status = 0)
    {
        $this->http_status = $http_status;
        $this->detail      = $message;
        // Pass $message as BOTH $a (so the lang string's {$a} renders the real cause)
        // and debuginfo (for server logs). Previously it went only to debuginfo, so
        // the friendly message showed a literal "{$a}" and the real error was lost.
        parent::__construct('error_api_connection', 'local_edzaiaxisfront', '', $message, $message);
    }
}
...
class axis_client
{
    ...
    public function ingest_file(string $filename, string $bytes, string $mimetype, array $form): array
    {
        ...
        if ($status >= 400) {
            ...
            throw new axis_client_exception("API error (HTTP {$status}) at /ingest/file: {$detail}", $status);
        }
        return $decoded;
    }
