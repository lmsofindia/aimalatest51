...
class api extends \external_api
{
    ...
    public static function get_cm_outputs(int $cmid): array
    {
        ...
        if (in_array('quiz', $show)) {
            ...
            $questions = [];
            foreach ($rows as $r) {
                // Skip malformed rows with no question text (legacy/broken sync data) so the
                // student UI never renders blank numbered questions.
                if (trim((string) $r->question_text) === '') { continue; }
                $questions[] = [
                    'id'            => $r->id,
    ...
    public static function save_quiz_edit(int $cmid, string $questions_json): bool
    {
        ...
        foreach ($questions as $i => $q) {
            $DB->insert_record('local_edzaiaxisfront_quiz_questions', (object)[
                ...
                'correct_answer' => trim($q['answer'] ?? $q['correct_answer'] ?? ''),
