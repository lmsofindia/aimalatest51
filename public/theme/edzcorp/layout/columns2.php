...
if (empty($editmodebutton) && $PAGE->user_allowed_editing()) {
    ...
    // Toggle edit mode through core's /editmode.php, which works on EVERY page
    // type (course, dashboard, admin, ...): it sets $USER->editing and redirects
    // back to pageurl. A plain ?edit=on link only works on /course/view.php; on
    // pages like /my/indexsys.php it is silently ignored, so edit mode never
    // engaged there — and with no edit mode, "Add a block" stayed hidden. The
    // old /course/view.php?id=<course> form ALSO redirected dashboards to the
    // site home (id 1). /editmode.php fixes both problems in one place.
