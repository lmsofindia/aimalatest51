 ...
 * @return url
 */
function theme_edzsaas_frontpage_section()
{
    ...
    if (!empty($theme->settings->frontpagenavlink)) {
        $templatecontext['frontpagenavlink'] = theme_edzsaas_header_links($theme->settings->frontpagenavlink, false);
        $templatecontext['frontpagenavlink-m'] = theme_edzsaas_header_links($theme->settings->frontpagenavlink, true);
    } else {
 ...
 * @return url
 */
function theme_edzsaas_frontpagecolor()
{
    $theme = theme_config::load('edzsaas');
 ...
 * @return url
 */
function edzsaas_get_course_image($id)
{
    global $CFG;
    $url = '';
 ...
 * @return url
 */
function edzsaas_get_blog_post_image($id)
{
    global $CFG;
    $url = '';
    require_once($CFG->libdir . '/filelib.php');
 ...
 * @return url
 */
function edzsaas_get_user_image($id)
{
    global $CFG;
    $url = '';
    require_once($CFG->libdir . '/filelib.php');
    ...
    $fs = get_file_storage();
    $files = $fs->get_area_files($context->id, 'user', 'icon', 'edzsaas', false);
    foreach ($files as $f) {
        if ($f->is_valid_image()) {
            $url = moodle_url::make_pluginfile_url(
                $f->get_contextid(),
 ...
 * @return url
 */
function theme_edzsaas_frontpageblockcategory($id)
{
    global  $DB;
 ...
 * @return content.
 */
function theme_edzsaas_links($links)
{
    $weblink = $links;
    $content = "";
    $websettings = explode("\n", $weblink);
 ...
 * @return content.
 */
function theme_edzsaas_header_links($links, $mobil)
{
    $weblink = $links;
    $content = "";
    $lurl = "";
 ...
 * @return randcolor
 */
function theme_edzsaas_random_color()
{
    /*
...
/**
 * Get the course overview image URL without using get_course().
 *
 * This function builds a course list element and attempts to extract the first valid image
 * from the course overview files.
 *
 * @param stdClass $course The course record containing at least id, fullname, category, etc.
 * @return string|false The image URL if found, false otherwise.
 *
 * @author Rashid
 * @date 5 Aug 2025
 */
function theme_edzsaas_get_course_image_search($course)
{
    global $CFG;
